  ...
  return (
    <>
      {/* Hero — one full viewport plus the slice of the wordmark watermark that
          hangs below the fold (see `.signin-hero` in Home.css), so the
      ...
      <div className="signin-hero relative flex min-h-[calc(100vh_+_var(--wordmark-hidden))] flex-col overflow-hidden">
        {/* Faint background grid lines. They start below the fixed nav bar
            ...
            from the top of the page (its own height plus the gap above it), and
            the lines begin half again as far down so they clear it with room to
            spare instead of running up behind it.
        ...
        {/* Centered sign-in content. The top padding clears the fixed nav bar,
            which no longer takes up space in this column, and the bottom
      ...
      {/* Tagline, sitting just below the giant wordmark at the base of the
          hero. It always reads as exactly two lines, broken after "for" so the
          pill opens the second line — each line is its own `nowrap` block, and
          the type scales with the viewport so the longer line always fits
          across instead of wrapping (see `.signin-tagline` in Home.css).
      ...
      <section className="px-4 py-20 sm:py-28">
        <p className="signin-tagline mx-auto text-center font-medium leading-[1.6] tracking-tight text-fg">
          <span className="block whitespace-nowrap">
            A shared office planning tool for
          </span>
          <span className="block whitespace-nowrap">
            <span className="signin-tagline-pill inline-flex items-center rounded-full bg-[#9E3B20] align-middle font-bold leading-[1.6] text-fg-inverse">
      ...
      {/* Blank breathing room below the tagline, sized off the viewport so the
          gap keeps its proportions on any screen. */}
