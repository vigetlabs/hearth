...
export function useCalendarOfficeSelection(
  ...
  const offices = useMemo(
    () => officesQuery.data ?? EMPTY_OFFICES,
    [officesQuery.data],
  );

  const defaultOffice = useMemo(
    () => findCurrentUserOffice(offices, user),
    [offices, user],
  );

  const requestedOfficeId = resolveRequestedOfficeId({
    searchParams,
    offices,
    user,
  });
...
function resolveRequestedOfficeId({
  ...
  offices,
  user,
}: ResolveRequestedOfficeIdOptions): number | undefined {
  ...
  const isDefaultOfficeRemote = user?.office?.name.toLowerCase() === "remote";

  if (isDefaultOfficeRemote) {
    return offices.find((office) => office.name.toLowerCase() !== "remote")?.id;
  }
...
function findCurrentUserOffice(
  ...
  return (
    offices.find((office) => office.id === user.office?.id) ??
    offices[0] ??
    null
  );
}
...
function findActiveOffice(
  ...
  return offices.find((office) => office.id === requestedOfficeId) ?? null;
}
